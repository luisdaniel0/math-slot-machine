# Starwake — decision archive

**This is the measurement history, not the instructions.** It is the record of how every
number in the game was arrived at, Jul 27 – Aug 13 2026. Most entries cost a sim run to
produce and would cost another to re-derive.

**GREP IT, DO NOT READ IT.** Search for the knob you are about to touch — a config key,
a mode name, a tier, a metric — before changing anything in the math. The live
instructions, traps and constraints are in `CLAUDE.md` (this directory); anything there
supersedes anything here.

⚠ **ORDERING IS APPROXIMATELY NEWEST-FIRST AND NOT RELIABLE.** The file was appended to
across many sessions and drifted: Aug 5 entries sit above Aug 8 ones, and a
"NEXT SESSION STARTS HERE (as of Jul 30)" survives near the bottom. **Trust the DATE IN
THE HEADING, not the position.** Where two entries disagree, the later date wins — most
say so explicitly, but not all.

⚠ Several entries were superseded wholesale and say so in their first lines; several
others were RETRACTED the same day they were written. Read the whole entry before acting
on a number from it. Notably: displayed mystery odds are **35/35/20/10** — every
35/30/25/10 below is stale; and the multiplier ladder that much of this file discusses at
length is **dead code** (see CLAUDE.md).

Design doc: `docs/ideas/starwake.md`. Competitor sheet: `BENCHMARKS.md`.
...
that **cost-adjusted volatility reads corvus 1.85 against ursa 1.88**, i.e. the menu's two
cheapest products were statistically the same product at 200x and 300x. Both are single
2x2 beasts and neither can reach 25,000x inside buy_mystery. The corvus TIER is untouched
and still rolls in base, ante_starfall, buy_mystery and buy_mystery_spin: **the tier was
never broken, selling it was.** As a free trigger, flat is correct.

Ursa survives because it is the coin flip -- 63.8% completion paying 1.66x the ticket --
and corvus completes 95.8% for 1.04x, which is your stake back and change.

THE MECHANIC. One spin. The set is dealt COMPLETE and the beast is woken before spin 1;
which beast is the mystery (corvus / ursa / draco, mix 15/25/60). Ascendant is excluded on
purpose: at one 2x2 block it is draco in a hat, and twin dragons is its whole identity.

⚠⚠ **WAKE VARIANTS CANNOT BE KEYED BY SCATTER COUNT. THE FIRST DESIGN WAS WRONG.**
Keying them at counts 7-10, the way ascendant is keyed at 6, is unbuildable:
`_force_special_board` places **AT MOST ONE SCATTER PER REEL**, so on five reels a 6th
already needs a reel window revealing two (the entire reason the ASC strip exists) and a
7th needs two such reels. In Python an unreachable forced count does not error -- it is a
bare `while True` and **hangs forever**. Implemented instead as a `wake` boolean on the
DISTRIBUTION, following the forceAscension path exactly, so the trigger board stays
ordinary (3/4/5 stars) and the flag changes only the feature.

⚠ THE PYTHON ENGINE CANNOT RUN THIS MODE and now says so. `constellation.py` predates act
two -- no star drops, no collection -- so it would silently write a full-length ladder
feature. `game_override.update_freespin_amount` raises NotImplementedError on a wake
condition rather than producing plausible wrong books.

MEASURED (unforced, Go engine, 1e6 unless noted):

    ROAMCAP    raw_mean   natural max   price @richness 2.2   richness @200x
    0           159.3x        10,047x           75x                0.82
    2           285.4x        17,496x          134x                1.48
    5           315.6x        17,496x          148x                1.63
    10          328.8x        17,496x          155x                1.70
    20          336.4x        17,496x          158x                1.74
    40          340.7x        17,496x          160x                1.76

⚠ **200x IS NOT BUILDABLE AT ANY ENRICHMENT.** Richness tops out at 1.76 against the
healthy 1.9-2.6, and a pool that thin leaves the optimizer nothing to discard -- which is
exactly how a mode ends up FLAT. **The cheaper ticket is what buys the volatility.**
Settled at **150x / ROAMCAP 10**, richness 2.27 (raw_mean 329.6x at 3e6).

⚠ **25,000x IS OUT OF STRUCTURAL REACH FOR ONE SPIN.** Natural max is **19,778x at 3e6,
identical across ROAMCAP 10 / 40 / 100 / 400 / 2000** -- enrichment moves the mean (+5%)
and does not move the ceiling at all. Same wall as "a denser roam strip cannot reach
25,000x". Published ceiling is **15,000x**, the first mode not to carry the headline
number. (The 17,496x repeated above is a SAMPLE, not a limit -- it moved to 19,778x at
3e6. Do not read a repeated max as a structural bound.)

MARKET CHECK, and it cuts both ways: Miko (all 7 modes 10,000x) and Rage Bait (all 6 modes
25,000x) both have every mode reach the game cap, INCLUDING their single-spin ones. But
Rage Spins does it by dealing **3-6 roaming wilds**, not one, and Miko Spin does it because
Miko's whole game caps at 10,000x. Keeping one 2x2 was a deliberate product call. At a 150x
ticket our 15,000x is **100x cost -- better than Miko Spin's 40x and Rage Spins' 71.4x**.

⚠ **IT DOES NOT BUST: zero-pay is 0.00% at every roam density.** The prediction that it
would (act two consumes the sticky wilds, so a woken deal has only the block) is WRONG in
practice -- a 2x2 wild on a 20-line 5x4 nearly always completes some line. This is the
MIKO shape (0.00% bust), not rage_spins' 5.95%, and it keeps the market norm intact.

⚠ **CAP-SLICE COST, AND ENRICHMENT DOES NOT FIX IT** (20k, quota 0.004):

    cap        ROAMCAP   redraws/book   books/s
    15,000        40         1,153        1,000     <- shipped
    15,000       200         1,287        1,009     <- enrichment made it WORSE
    13,500        40           522        2,088
    12,000        40           236        4,885
    12,000       200           191        5,990

Against 1.31 redraws/book unforced. The slice hit its 0.4000% quota exactly in every
variant, so it is expensive rather than failing, but 15,000x is ~76% of the best board
ever observed and that is the "manufacture a near-best-case round every time" condition.
**THE CAP TARGET IS THE ONLY LEVER; ROAM DENSITY IS NOT.** Dropping to 12,500x is a
one-line change if the ~17min/1e6 generation cost becomes annoying.

Cap share `spin_cap_rtp` 0.05 -> max-win rate 1 in 2,000 (rate = slice_rtp*cost/cap),
inside the 1-in-400-to-4,000 market band and near rage_spins' 1 in 1,377.

TWO MYSTERY BUYS IS PRECEDENTED. Rage Bait ships `bonus_fs_mystery` (500x, 0.00% bust,
sigma 2.68) and `bonus_fs_mystery_ante` (500x, **46.52% bust**, sigma 3.32) side by side --
same RTP, same cap, separated purely by bust-vs-tail shape. Ours separate on LENGTH.

FALLOUT TO WATCH: corvus's forced ascension went with buy_corvus, so ascension now
delivers at its natural rate everywhere. **The rules screen's "1 in 634" described
buy_corvus alone and is STALE.**
